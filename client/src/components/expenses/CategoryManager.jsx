...
import { Modal, Button, TextInput, ConfirmDialog } from "../ui.jsx";
...
export default function CategoryManager({ onClose }) {
	...
	const [confirmDeleteCat, setConfirmDeleteCat] = useState(null); // {id, count}
	...
	const removeCat = (catId) => {
		...
		if (n > 0) {
			setConfirmDeleteCat({ id: catId, count: n });
			return;
		}
		...
		<>
			...
			{confirmDeleteCat && (
				<ConfirmDialog
					...
					message={`Delete ${confirmDeleteCat.count} expense${confirmDeleteCat.count > 1 ? "s" : ""} in this category?`}
					confirmLabel="Delete"
					onConfirm={() => {
