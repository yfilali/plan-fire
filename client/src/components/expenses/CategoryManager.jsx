...
export default function CategoryManager({ categories, setCategories, expenses, setExpenses, onClose }) {
	const S = useTheme();
	const [draft, setDraft] = useState({ label: "", icon: "📂", color: COLOR_OPTIONS[0] });

	const addCat = () => {
		if (!draft.label.trim()) return;
		setCategories((p) => [...p, { id: uid(), label: draft.label.trim(), icon: draft.icon, color: draft.color }]);
					...
					{EMOJIS.map((em) => (
						<button
							...
							style={{
								...
								borderRadius: 8,
								fontSize: 15,
								border: draft.icon === em ? `2px solid ${S.accent}` : `1px solid ${S.border}`,
								background: draft.icon === em ? S.accent + "22" : "transparent",
							}}
						>
							{em}
						</button>
					...
					{COLOR_OPTIONS.map((c) => (
						<button
							...
							style={{
								...
								borderRadius: 12,
								background: c,
								border: draft.color === c ? `2px solid ${S.text}` : "2px solid transparent",
							}}
						/>
					))}
				</div>
				...
				{categories.map((cat) => (
					...
					>
						...
						<button
							...
							style={{ ...btnBase, padding: "3px 9px", borderRadius: 6, background: "transparent", color: S.danger, fontSize: 11, border: `1px solid ${S.border}` }}
						>
							✕
						</button>
					</div>
