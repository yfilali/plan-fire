...
describe('monthlySpendAtAge', () => {
  ...
  const baseExpenses = [
    { id: 'a', cat: 'food', name: 'Groceries', amount: 800, scenarios: ['all'], inflOverride: null },
    { id: 'b', cat: 'housing', name: 'Mortgage', amount: 2400, scenarios: ['stay'], inflOverride: 0 },
  ];
  ...
  it('applies per-expense inflation override when set', () => {
    const expenses = [
      { id: 'a', cat: 'health', name: 'ACA', amount: 1500, scenarios: ['all'], inflOverride: 0.065 }, // healthcare premium inflation
      { id: 'b', cat: 'food', name: 'Groceries', amount: 800, scenarios: ['all'], inflOverride: null },
    ];
...
describe('project — inflation-aware with nominal returns', () => {
  const baseExpenses = [
    { id: 'a', cat: 'food', name: 'Groceries', amount: 800, scenarios: ['all'], inflOverride: null },
    { id: 'b', cat: 'housing', name: 'Mortgage BA', amount: 2400, scenarios: ['stay'], inflOverride: 0 },
    { id: 'c', cat: 'health', name: 'ACA', amount: 1500, scenarios: ['all'], inflOverride: 0.065 },
  ];
  ...
  it('grows portfolio at nominal return when no returns array is passed', () => {
    ...
    const result = project({
      startAge: 49, endAge: 58, retireAge: 49, ssAge: 70, ssAnnual: 0,
      portfolio: 3000000, expenses: baseExpenses, scenario: 'stay',
      nomReturn: 0.07, inflation: 0.03, rentalNet: 0,
    });
  ...
  it('inflates different expense categories at their own rates over time', () => {
    const result = project({
      startAge: 49, endAge: 59, retireAge: 49, ssAge: 70, ssAnnual: 0,
      portfolio: 3000000, expenses: baseExpenses, scenario: 'stay',
      nomReturn: 0.07, inflation: 0.03, rentalNet: 0,
    });
  ...
  it('handles lost decade returns array with per-expense inflation', () => {
    const result = project({
      startAge: 49, endAge: 58, retireAge: 49, ssAge: 70, ssAnnual: 0,
      portfolio: 3000000, expenses: baseExpenses, scenario: 'stay',
      nomReturn: buildReturns('lost_decade', 0.07), inflation: 0.03, rentalNet: 0,
    });
  ...
  it('retirement age stops deductions from portfolio when income covers expenses', () => {
    ...
    const expenses = [
      { id: 'a', cat: 'food', name: 'Groceries', amount: 8000, scenarios: ['all'], inflOverride: null },
      { id: 'b', cat: 'health', name: 'ACA', amount: 5000, scenarios: ['all'], inflOverride: 0.065 },
    ];
    const result = project({
      startAge: 49, endAge: 58, retireAge: 52, ssAge: 70, ssAnnual: 0,
      portfolio: 1500000, expenses, scenario: 'stay',
      nomReturn: 0.07, inflation: 0.03, rentalNet: 0,
    });
